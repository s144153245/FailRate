...
def extract_repair_errorcode(html: str) -> str:
    ...
    def _find_table_with_headers(required_headers: set[str]):
    ...
    def _extract_from_production_history(prod_table) -> str:
        ...
        for cells in search_rows:
            ...
            if consecutive_fail >= 3 and consecutive_candidate:
                ...
                print(prev_stage,consecutive_candidate)
        if consecutive_code:
...
def _errorcode_summary(
    ...
    """Count errorcodes for a given subset (mask) using last record per SN only."""
...
def run_yield_and_errorcode_summary(log):
...
class App:
    def __init__(self, root):
        self.root = root
        self.root.title("Testing Summary Tool")

        r1 = tk.Frame(root)
        r1.pack(fill="x", padx=10, pady=6)
        tk.Button(r1, text="Run Summary (Yield + ErrorCodes)", command=self.run_summary).pack(side="left", padx=5)
        tk.Button(r1, text="Clear Prompt", command=self.clear_log).pack(side="right", padx=5)

        r2 = tk.Frame(root)
        r2.pack(fill="both", expand=True, padx=10, pady=6)
        self.log_text = tk.Text(r2, height=22, state="disabled")
    ...
    def run_summary(self):
        try:
            run_yield_and_errorcode_summary(self.log)
        except Exception as e:
            self.log(f"ERROR: {e}")
